...
func (l *Log) wait() {
	...
	if l.backoff < l.conf.Max {
		l.backoff = l.backoff * time.Duration(l.conf.Factor)
		if l.backoff > l.conf.Max {
			l.backoff = l.conf.Max
		}
	}
}
